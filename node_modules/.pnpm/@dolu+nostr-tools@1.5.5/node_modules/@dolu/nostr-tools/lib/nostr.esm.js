...
async function finishEvent(t, privateKey) {
  let event = t;
...
async function verifySignature(event) {
  return secp256k12.schnorr.verify(
    event.sig,
    getEventHash(event),
    event.pubkey
  );
...
async function signEvent(event, key) {
  return secp256k12.utils.bytesToHex(
    await secp256k12.schnorr.sign(getEventHash(event), key)
  );
}
    ...
    return new Promise((resolve, reject) => {
      ...
      async function handleNext() {
        ...
        if (subid) {
          ...
          if (so && so.alreadyHaveEvent && await so.alreadyHaveEvent(getHex64(json, "id"), url)) {
            return;
          }
        }
        try {
          ...
          switch (data[0]) {
            case "EVENT":
              let id = data[1];
              let event = data[2];
              if (validateEvent(event) && openSubs[id] && (openSubs[id].skipVerification || await verifySignature(event)) && matchFilters(openSubs[id].filters, event)) {
                openSubs[id];
